...
AppType nameToAppType(const std::string& name) {
    if (name.empty()) return AppType::UNKNOWN;
    
    std::string lower_name = name;
    std::transform(lower_name.begin(), lower_name.end(), lower_name.begin(),
                   [](unsigned char c) { return std::tolower(c); });
                   
    if (lower_name == "http") return AppType::HTTP;
    if (lower_name == "https") return AppType::HTTPS;
    if (lower_name == "dns") return AppType::DNS;
    if (lower_name == "tls") return AppType::TLS;
    if (lower_name == "quic") return AppType::QUIC;
    if (lower_name == "ssh") return AppType::SSH;
    if (lower_name == "google") return AppType::GOOGLE;
    if (lower_name == "facebook") return AppType::FACEBOOK;
    if (lower_name == "youtube") return AppType::YOUTUBE;
    if (lower_name == "twitter" || lower_name == "x") return AppType::TWITTER;
    if (lower_name == "instagram") return AppType::INSTAGRAM;
    if (lower_name == "netflix") return AppType::NETFLIX;
    if (lower_name == "amazon") return AppType::AMAZON;
    if (lower_name == "microsoft") return AppType::MICROSOFT;
    if (lower_name == "apple") return AppType::APPLE;
    if (lower_name == "whatsapp") return AppType::WHATSAPP;
    if (lower_name == "telegram") return AppType::TELEGRAM;
    if (lower_name == "tiktok") return AppType::TIKTOK;
    if (lower_name == "spotify") return AppType::SPOTIFY;
    if (lower_name == "zoom") return AppType::ZOOM;
    if (lower_name == "discord") return AppType::DISCORD;
    if (lower_name == "github") return AppType::GITHUB;
    if (lower_name == "cloudflare") return AppType::CLOUDFLARE;
