...
uint16_t PayloadInspector::readUint16BE(const uint8_t* data) {
    return (static_cast<uint16_t>(data[0]) << 8) | data[1];
}

uint32_t PayloadInspector::readUint24BE(const uint8_t* data) {
    return (static_cast<uint32_t>(data[0]) << 16) |
           (static_cast<uint32_t>(data[1]) << 8) |
           data[2];
}

bool PayloadInspector::isTLSClientHello(const uint8_t* payload, size_t length) {
...
std::optional<std::string> PayloadInspector::extract(const uint8_t* payload, size_t length) {
    ...
    (void)handshake_length; // Suppress unused warning
    offset += 4;
...
std::vector<std::pair<uint16_t, std::string>> PayloadInspector::extractExtensions(
    const uint8_t* payload, size_t length) {
    std::vector<std::pair<uint16_t, std::string>> extensions;
    // (Abbreviated implementation for compilation)
    return extensions;
}
...
bool HttpHostInspector::isHTTPRequest(const uint8_t* payload, size_t length) {
...
std::optional<std::string> HttpHostInspector::extract(const uint8_t* payload, size_t length) {
    ...
    (void)host_header; // Suppress unused variable warning
...
bool DnsInspector::isDNSQuery(const uint8_t* payload, size_t length) {
...
std::optional<std::string> DnsInspector::extractQuery(const uint8_t* payload, size_t length) {
...
bool QuicInspector::isQUICInitial(const uint8_t* payload, size_t length) {
...
std::optional<std::string> QuicInspector::extract(const uint8_t* payload, size_t length) {
    ...
    for (size_t i = 0; i + 50 < length; i++) {
        if (payload[i] == 0x01) {
            auto result = PayloadInspector::extract(payload + i - 5, length - i + 5);
